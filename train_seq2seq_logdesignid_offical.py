...
__description__ = "基于action sequence的seq2seq模型训练 (official version)"
...
#                            logdesignid_data_enc.append(self.logdesignid_vocab_to_int['<EOS>'])
    ...
    def encoding_layer(self, enc_embed_input, rnn_size, num_layers,
        ...
        with tf.variable_scope("encode"):
            ...
            # Build RNN cell
            encoder_cell = tf.nn.rnn_cell.BasicLSTMCell(rnn_size)

            # Run Dynamic RNN
            #   encoder_outputs: [max_time, batch_size, num_units]
            #   encoder_state: [batch_size, num_units]
            enc_output, enc_state = tf.nn.dynamic_rnn(
                encoder_cell, enc_embed_input,
...
#        dec_embed_input = tf.nn.embedding_lookup(dec_embeddings, dec_input)
        ...
        with tf.variable_scope("decode"):
            ...
            projection_layer = Dense(
                target_vocab_size, use_bias=False)

            # Helper
            helper = tf.contrib.seq2seq.TrainingHelper(
                dec_embed_input, target_sequence_length, time_major=False)
            # Decoder
        ...
        return training_decoder_output, logits
...
#        dec_input = self.process_decoder_input(targets)
        ...
        training_decoder_outputs, training_decoder_logits = self.decoding_layer(self.logdesignid_vocab_to_int,
                                                      num_layers, 
                                                      lstm_size,
        ...
        return training_decoder_outputs, training_decoder_logits, enc_state
    ...
    def model_train(self):
        ...
        with train_graph.as_default():
            ...
            embedding_encoder = tf.get_variable(
                "embedding_encoder", [len(self.logdesignid_int_to_vocab), self.embedding_size_logdesignid])
            ...
            embedding_decoder = tf.get_variable(
                "embedding_decoder", [len(self.logdesignid_int_to_vocab), self.embedding_size_logdesignid])
...
#            targets = self.player_logid_test
            ...
            # Create the training and inference logits
                training_decoder_outputs, logits, enc_state = self.seq2seq_model(input_data_enc,
                                                             input_data_dec,
                                                             target_logdesignid,
                                                             lr,
                                                             source_sequence_length,
